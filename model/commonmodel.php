...
//        SELECT SUM(weight) as tw FROM `bintrans` GROUP BY binAllocationID;
        //Execute a query
        $result = $con->query($sql);
        return $result;
    }
...
class common {
    ...
    public function SumBinWeight() {
        $con = $GLOBALS['con'];
        //sql query
        $sql = "SELECT SUM(weight) as tw,binAllocationID FROM `bintrans` GROUP BY binAllocationID";
        //Execute a query
        $result = $con->query($sql);
        return $result;
    }
